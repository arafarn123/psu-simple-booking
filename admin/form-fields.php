...
<?php
// Helper method สำหรับแสดงตัวอย่างฟิลด์
function render_field_preview($field) {
    $output = '';
    
    switch ($field['type']) {
        case 'textarea':
            $output = '<textarea class="psu-textarea" placeholder="' . esc_attr($field['placeholder']) . '" disabled></textarea>';
            break;
        case 'select':
            $output = '<select class="psu-select" disabled><option>เลือก...</option>';
            if ($field['options']) {
                $options = explode("\n", $field['options']);
                foreach ($options as $option) {
                    $option = trim($option);
                    if ($option) {
                        $output .= '<option>' . esc_html($option) . '</option>';
                    }
                }
            }
            $output .= '</select>';
        ...
        case 'radio':
            if ($field['options']) {
                ...
                foreach ($options as $option) {
                    $option = trim($option);
                    if ($option) {
                        $output .= '<label><input type="radio" disabled> ' . esc_html($option) . '</label><br>';
                    }
                }
            }
            break;
        case 'checkbox':
            if ($field['options']) {
                ...
                foreach ($options as $option) {
                    $option = trim($option);
                    if ($option) {
                        $output .= '<label><input type="checkbox" disabled> ' . esc_html($option) . '</label><br>';
                    }
                }
            }
            break;
        default:
            $output = '<input type="' . esc_attr($field['type']) . '" class="psu-input" placeholder="' . esc_attr($field['placeholder']) . '" disabled>';
    }
